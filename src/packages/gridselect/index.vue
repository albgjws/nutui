...
<template>
  <div class="grid-select">
    <lc-option
      ...
      @select="onSelect(index)"></lc-option>

    <lc-option
      v-for="(item, index) in cHackList"
      v-bind="Object.assign({}, customStyles, item)"
      :key="'cHackList'+index"
      :style="{opacity: 0, marginTop: dList.length >= cols ? lineSpacing : null}"></lc-option>
  </div>
</template>

<script>
  import LcOption from './option.vue';

  export default {
    components: { LcOption },
    name:"nut-grid-select",
    props: {
      // 列数
      cols: {
        type: Number,
