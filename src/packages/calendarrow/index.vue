    ...
    <div>
       <div class="bgc-gradient-blue">
           <div class="align-center justify-center cal-wrap">
                <span @click="isVisible = !isVisible" class="fs36 f-white">{{timeTitle}}</span>
           </div>
           <div class="daily-week">
               <div class="flex-row">
                   <div v-for="(item,index) in weekArr" :key="index" class="row-item align-center"><span class="fs24 f-white">{{item.label}}</span></div>
               </div> 
               <div flex-direction="row" class="flex-row">
                  <nut-swiper class="slider"
                      direction="horizontal"
                      @slideMove="scrollHandler" @slideChangeEnd="changeHandler">
                    <!-- <slider class="slider" interval="3000" @scroll="scrollHandler" @change="changeHandler"> -->
                        <div class="slider-pages" v-for="(item,i) in daysArr" :key="i">
                            <div class="row-item justify-center align-center"  v-for="(day, index) in days" :key="index" @click="picktime(day,index)">
                                <!--本月-->
                                <span v-if="day.getMonth()+1 != currentMonth" class="other-month fs28 f-white weeknum mt25 opacity-5" :class="[index==isActIndex&&'active']">{{ day.getDate() }}</span>
                                <div v-else>
                                    <!--今天-->
                                    <span v-if="day.getFullYear() == new Date().getFullYear() && 
                                                day.getMonth() == new Date().getMonth() && 
                                                day.getDate() == new Date().getDate()" class="fs28 f-white weeknum mt25 current" :class="[index==isActIndex&&'active']">{{ day.getDate() }}</span>
                                    <span v-else class="fs28 f-white weeknum mt25" :class="[index==isActIndex&&'active']">{{ day.getDate() }}</span>
                                </div>
                            </div>
                        </div>
                    <!-- </slider> -->
